...
impl EngineRunner {
    ...
    /// Exports Engine
    pub fn export(&mut self, engine: &mut Engine) {
        ...
        match engine.config.export_variant {
            ExportVariant::ToFile => self.export_to_file(engine),
            ExportVariant::ToDatabase => self.export_to_database(engine),
        }
    ...
    pub fn export_to_database(&self, engine: &mut Engine) {
